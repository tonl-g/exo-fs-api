...
if (process.argv.length !==6) {
    console.log(`usage: node append.js src1.txt src2.txt src3.txt dst.txt`)
    process.exit(1)
}
...
if (!fs.existsSync(process.argv[2])) {
    ...
    process.exit(1)
} else if (!fs.existsSync(process.argv[5])) { 
    console.log(`Error: ${process.argv[5]} does not exist`) 
    process.exit(1)
}

copyFileSync('./src1.txt', './dst.txt') 
copyFileSync('./src2.txt', './dst.txt') 
copyFileSync('./src3.txt', './dst.txt')
...
console.log(`Les contenus de src1.txt ${contentSrc1}, src2.txt ${contentSrc2}, src3.txt ${contentSrc3} et le contenu de dst.txt ${contentDst}`)
